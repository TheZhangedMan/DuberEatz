 ...
 * The PPM file that this code produces will place a black tile in the place of a microwave.
 */

public class DuberEatzLevelFourPlus {
  ...
  public static void main(String[] args) throws Exception {
    ...
    int result = solve(map, visits, yCoor, xCoor, 0, 0);
    outputPPM.print("P3\n" + (width * 10) + " " + (length * 10) + "\n255\n");
    for (int i = 0; i < length; i++) {
      for (int j = 0; j < 10; j++) { //The length of each PPM map tile is ten pixels.
        for (int k = 0; k < width; k++) {
          for (int l = 0; l < 10; l++) { //The width of each PPM map tile is ten pixels.
            if (trail[i][k] == true) { //If a tile has been visited, mark it as part of the trail to compensate for erased trails.
              optimal[i][k] = "x";
            }
 ...
 /**
 * solve
 * This method recursively solves the provided maze, saving the optimal path for tips and its trail to the global variables.
 ...
 */
  ...
  public static int solve(String[][] map, boolean[][] visits, int yCoor, int xCoor, int moves, int tips) {
    ...
    if (identity == true) {
      ...
      if (Integer.parseInt(dupe[yCoor][xCoor]) - moves <= 0) {
        tips += Integer.parseInt(dupe[yCoor][xCoor]) - moves; //Calculate the amount of tips provided from destination and add to total number of tips.
      } else {
      ...
      if (completion == true) {
        ...
        return solve(dupe, track, yCoor, xCoor, moves, tips); //Then, recursively call itself again with its position at the reached destination.
      }
    }
    if (!((dupe[yCoor][xCoor].equals("X")) || (dupe[yCoor][xCoor].equals("S")))) {
    ...
    if (!((dupe[yCoor - 1][xCoor].equals("#")) || (dupe[yCoor - 1][xCoor].equals("x")) || (dupe[yCoor - 1][xCoor].equals("X")) || (dupe[yCoor - 1][xCoor].equals("S")))) {
      up = solve(dupe, track, yCoor - 1, xCoor, moves + 1, tips); //Traverse to tile above.
    }
    if (!((dupe[yCoor + 1][xCoor].equals("#")) || (dupe[yCoor + 1][xCoor].equals("x")) || (dupe[yCoor + 1][xCoor].equals("X")) || (dupe[yCoor + 1][xCoor].equals("S")))) {
      down = solve(dupe, track, yCoor + 1, xCoor, moves + 1, tips); //Traverse to tile below.
    }
    if (!((dupe[yCoor][xCoor - 1].equals("#")) || (dupe[yCoor][xCoor - 1].equals("x")) || (dupe[yCoor][xCoor - 1].equals("X")) || (dupe[yCoor][xCoor - 1].equals("S")))) {
      left = solve(dupe, track, yCoor, xCoor - 1, moves + 1, tips); //Traverse to left tile.
    }
    if (!((dupe[yCoor][xCoor + 1].equals("#")) || (dupe[yCoor][xCoor + 1].equals("x")) || (dupe[yCoor][xCoor + 1].equals("X")) || (dupe[yCoor][xCoor + 1].equals("S")))) {
      right = solve(dupe, track, yCoor, xCoor + 1, moves + 1, tips); //Traverse to right tile.
    }
    return 0;
  }
